...
@st.cache_data(show_spinner=False)
def build_backtest_actuals_vs_predicted(
    head: str,
    model_kind: str,
    _bundle,
    ...
    """
    Run an expanding-window OOS backtest for a given head/model.
    Returns (actual_series_in_PKR, oos_df) where oos_df has columns
    [year, yhat] with values in PKR (not log, not billions).
    """
    ...
    df = _df_hist.copy()
    years = [p.year for p in df.index]
    total_obs = len(df)

    # Minimum training size: leave n_test folds for OOS
    ...
    for split in range(min_train, total_obs):
        ...
        test_year = df.index[split].year

        try:
            # ── Build a minimal 1-step-ahead exog for the test row ─────────
            # Use contemporaneous values from the actual test row where available
            exog_row = {}
            for col in spec_x:
                base = col.split(".L")[0] if ".L" in col else col
                # strip log_ prefix to get raw col if needed
                if base in df.columns:
                    exog_row[base] = float(df_test[base]) if not pd.isnull(df_test.get(base, np.nan)) else float(df_train[base].iloc[-1])
                elif col in df.columns:
                    exog_row[col] = float(df_test[col]) if not pd.isnull(df_test.get(col, np.nan)) else float(df_train[col].iloc[-1])

            # Build a 1-row exog DataFrame
            exog_df_1 = pd.DataFrame([exog_row])
            ...
            if model_kind == "ardl":
                ...
                for k, v in params.items():
                    ...
                    if k.startswith(y_name + ".L"):
                        ...
                        col_name, lag_str = k.rsplit(".L", 1)
                        exog_lags.setdefault(col_name, {})[int(lag_str)] = v
                    else:
                ...
                for lag, coef in ar_lags.items():
                    idx = split - lag
                    if idx >= 0:
                        yhat_log += coef * float(df.iloc[idx][y_name])
                for col_name, lag_dict in exog_lags.items():
                    for lag, coef in lag_dict.items():
                        idx = split - lag
                        if idx >= 0 and col_name in df.columns:
                            val = float(df.iloc[idx][col_name])
                            if not np.isnan(val):
                                yhat_log += coef * val
                ...
                params = dict(res.params)
                rho   = params.get("L1.log_tax", params.get("ar.L1", 0.0))
                const = params.get("const", params.get("intercept", 0.0))

                yhat_log = const + rho * float(df.iloc[split - 1][y_name])
                # Add exog contributions
                for k, v in params.items():
                    if k in ("const", "intercept", "L1.log_tax", "ar.L1", "sigma2"):
                        continue
                    if k in df.columns:
                        val = float(df.iloc[split][k]) if split < len(df) else float(df.iloc[-1][k])
                        if not np.isnan(val):
                            yhat_log += v * val
                ...
                for c in feat_cols:
                    if c.endswith("_L0"):
                        base = c[:-3]
                        row[c] = float(df_train[base].iloc[-1]) if base in df_train.columns else 0.0
                    elif "_L" in c:
                        parts = c.rsplit("_L", 1)
                        base_c, lag = parts[0], int(parts[1])
                        idx = split - lag
                        row[c] = float(df.iloc[idx][base_c]) if idx >= 0 and base_c in df.columns else 0.0
                    else:
                        row[c] = float(df_train[c].iloc[-1]) if c in df_train.columns else 0.0
                row_df   = pd.DataFrame([row], columns=feat_cols).fillna(0)
                yhat_log = float(model.predict(row_df)[0])
            else:
                continue
    ...
    # Actual series in PKR
    actual = np.exp(df[y_name].dropna())
    actual.index = pd.Index([p.year for p in actual.index], name="year")

    return actual, oos_df


@st.cache_data(show_spinner=False)
def build_insample_fitted(
    head: str,
    model_kind: str,
    _bundle,
    _df_hist: pd.DataFrame,
    ...
    """
    Compute in-sample one-step-ahead fitted values using the full trained model.
    Returns (actual_pkr, fitted_pkr) both indexed by fiscal year (int).
    """
    if _bundle is None or _df_hist is None:
            ...
            if model_kind == "ardl":
                ...
                for k, v in params.items():
                    if k in ("const", "intercept"):
                        continue
                    if k.startswith(y_name + ".L"):
                        ar_lags[int(k[len(y_name) + 2:])] = v
                    elif ".L" in k:
                        col_n, lag_s = k.rsplit(".L", 1)
                        exog_lags.setdefault(col_n, {})[int(lag_s)] = v
                    else:
                        exog_lags.setdefault(k, {})[0] = v
                ...
                for lag, coef in ar_lags.items():
                    ...
                    if idx >= 0:
                        yhat_log += coef * float(df.iloc[idx][y_name])
                for col_n, lag_dict in exog_lags.items():
                    for lag, coef in lag_dict.items():
                        idx = i - lag
                        if idx >= 0 and col_n in df.columns:
                            val = float(df.iloc[idx][col_n])
                            if not np.isnan(val):
                                yhat_log += coef * val
                ...
                rho    = params.get("L1.log_tax", params.get("ar.L1", 0.0))
                const  = params.get("const", params.get("intercept", 0.0))
                yhat_log = const + rho * float(df.iloc[i - 1][y_name])
                for k, v in params.items():
                    if k in ("const", "intercept", "L1.log_tax", "ar.L1", "sigma2"):
                        continue
                    if k in df.columns:
                        val = float(df.iloc[i][k]) if not np.isnan(float(df.iloc[i].get(k, np.nan))) else 0.0
                        yhat_log += v * val
                ...
                for c in feat_cols:
                    if c.endswith("_L0"):
                        base = c[:-3]
                        row[c] = float(df.iloc[i][base]) if base in df.columns and not np.isnan(float(df.iloc[i].get(base, np.nan))) else 0.0
                    elif "_L" in c:
                        parts = c.rsplit("_L", 1)
                        base_c, lag = parts[0], int(parts[1])
                        idx = i - lag
                        ...
                        row[c] = float(df.iloc[i][c]) if c in df.columns else 0.0
                row_df   = pd.DataFrame([row], columns=feat_cols).fillna(0)
                yhat_log = float(model.predict(row_df)[0])
            else:
                continue

            fitted_records.append({"year": years[i], "fitted": float(np.exp(yhat_log))})

        except Exception:
            continue

    fitted_s = pd.Series(
        {r["year"]: r["fitted"] for r in fitted_records},
        name="fitted"
    )
    actual_s = np.exp(df[y_name].dropna())
    actual_s.index = pd.Index([p.year for p in actual_s.index], name="year")

    return actual_s, fitted_s
...
_persisted = _load_2026_overrides()
...
with tab3:
    ...
    # ── Actual vs Backtest Predictions Chart ─────────────────────────────────
    if is_mm and bundle and df_hist is not None:
        ...
        with bt_col2:
            bt_model = st.selectbox(
                "Model",
                options=["ardl", "arimax", "enet"],
                format_func=lambda x: x.upper(),
                key="bt_model_select",
            )

        with st.spinner("Building out-of-sample backtest chart…"):
            bt_actual, bt_oos = build_backtest_actuals_vs_predicted(
                bt_head, bt_model, bundle, df_hist, n_test=10
            )
        ...
        if len(bt_actual) > 0:
            ...
            # OOS backtest dashed line
            if len(bt_oos) > 0:
                fig_bt.add_trace(go.Scatter(
                    x=bt_oos["year"].tolist(),
                    y=(bt_oos["yhat"] / 1000).tolist(),
                    mode="lines+markers",
                    name=f"Backtest ({bt_model.upper()})",
            ...
            fig_bt.update_layout(
                title=dict(
                    text=f"{TAX_LABELS[bt_head]} — Actual vs {bt_model.upper()} Backtest",
            ...
            st.caption(
                ...
                f"(10 expanding-window folds, n_test=10). No in-sample fitted values shown."
            )
        else:
    ...
    # ── In-Sample Fitted Values Chart ────────────────────────────────────────
    if is_mm and bundle and df_hist is not None:
        st.markdown("#### 📈 In-Sample Fitted Values")

        with st.spinner("Computing in-sample fitted values…"):
            is_actual, is_fitted = build_insample_fitted(
                bt_head, bt_model, bundle, df_hist
            )

        if len(is_actual) > 0 and len(is_fitted) > 0:
            fig_is = go.Figure()

            # Actual
            fig_is.add_trace(go.Scatter(
                x=list(is_actual.index),
                y=(is_actual / 1000).tolist(),
                mode="lines+markers",
                name="Actual",
                line=dict(color="#2563EB", width=2.5),
                marker=dict(size=5, color="#2563EB"),
            ...
            # In-sample fitted
            fig_is.add_trace(go.Scatter(
                x=list(is_fitted.index),
                y=(is_fitted / 1000).tolist(),
                mode="lines+markers",
                name=f"In-Sample Fit ({bt_model.upper()})",
                line=dict(color="#10B981", width=2, dash="dot"),
                marker=dict(symbol="diamond", size=6, color="#10B981"),
            ))

            fig_is.update_layout(
                title=dict(
                    text=f"{TAX_LABELS[bt_head]} — Actual vs In-Sample Fitted ({bt_model.upper()})",
                    font=dict(size=14, color="#1F2937"),
                    x=0,
                ),
                xaxis=dict(
            ...
            st.caption("Green dotted = one-step-ahead in-sample fitted values using full trained model on observed data.")
        else:
            st.info("In-sample fitted values not available for this combination.")
    ...
    if is_mm and bundle and df_hist is not None:
        ...
        with _fore_tbl_cols[1]:
            ft_model = st.selectbox(
                ...
                options=["ardl", "arimax", "enet"],
                format_func=lambda x: x.upper(),
                index=["ardl", "arimax", "enet"].index(bt_model),
                key="ft_model_select",
            )
        ...
        if ft_fore_df is not None and len(ft_fore_df) > 0:
            ...
            # Also append in-sample comparison rows (last 5 actuals vs fitted)
            _is_actual_ft, _is_fitted_ft = build_insample_fitted(ft_head, ft_model, bundle, df_hist)
            if len(_is_actual_ft) > 0 and len(_is_fitted_ft) > 0:
                _common_yrs = sorted(set(_is_actual_ft.index) & set(_is_fitted_ft.index))[-5:]
                _is_rows = []
                for yr in _common_yrs:
                    act_val  = _is_actual_ft[yr] / 1000
                    fit_val  = _is_fitted_ft[yr] / 1000
                    err_pct  = ((fit_val - act_val) / act_val * 100) if act_val != 0 else 0.0
                    _is_rows.append({
                        "Fiscal Year"    : yr,
                        "Actual (PKR bn)": round(act_val, 2),
                        "Fitted (PKR bn)": round(fit_val, 2),
                        "Error %"        : round(err_pct, 2),
                    })
                is_display = pd.DataFrame(_is_rows)

                c_fore, c_insample = st.columns([1, 1])
                with c_fore:
                ...
                with c_insample:
                    st.markdown(f"**In-Sample Fit (Last 5 Years) — {ft_model.upper()}**")
                    st.dataframe(
                ...
                st.markdown(f"**Forecast — {TAX_LABELS[ft_head]}**")
                st.dataframe(
                    ft_display.style.format({
                        "Forecast (PKR bn)": "{:,.2f}",
...
with tab7:
    ...
    with _p_col1:
        if _persist_exists:
            st.success("✅ Your 2026 edits are saved and will reload automatically on next run.", icon=None)
        else:
            st.info("ℹ️ No saved overrides yet — edit the 2026 row below to save.")
    with _p_col2:
